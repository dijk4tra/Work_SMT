/**
 * @file time_utils.cpp
 * @brief 实现服务端 UTC 时间生成和严格 ISO 8601 时间解析。
 */
...
ServerTime currentServerTime() {
    ...
    char iso8601[40];
    std::snprintf(iso8601, sizeof(iso8601), "%s.%03dZ", base, millisecond_part);

    std::strftime(base, sizeof(base), "%Y-%m-%d %H:%M:%S", &utc);
    char mysql[40];
    std::snprintf(mysql, sizeof(mysql), "%s.%03d", base, millisecond_part);
    return ServerTime{iso8601, mysql};
}
