...
`collector_id` 必须与 HMAC 认证得到的 `device_id` 存在启用绑定。绑定规则从第三期迁移和创建
上传接口开始执行，并回补到心跳接口；绑定不存在或停用时返回 HTTP 403
`COLLECTOR_DEVICE_MISMATCH`。
...
| 403 | `DEVICE_DISABLED`、`COLLECTOR_DEVICE_MISMATCH`、`UPLOAD_DEVICE_MISMATCH` |
