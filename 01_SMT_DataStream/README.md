...
项目采用六期连续开发，六期全部完成后交付第一期生产终板。当前第一至第四期已经完成，第五期
参考采集程序代码和测试已生成，最终三线网络闭环因本机提权额度限制待复跑。
...
第五期 E2E 已注册到 CTest，但当前尚未实际启动本机网络进程，不能把该项写成已通过。
...
后续代码使用三条生产线、每线一台 IPC 和 SPI/AOI/FCT 的固定仿真目标；当前目录中的单线 ICT
数据是已完成阶段的历史样本，将在第三、五期通过新迁移、种子和增强生成器替换。业务规模与
文件行为分别见 [`docs/模拟业务基线.md`](docs/模拟业务基线.md) 和
[`docs/采集端与文件生命周期契约.md`](docs/采集端与文件生命周期契约.md)。
