...
当前版本已完成六阶段实现，目标版本为 `v1.0.0`。Search Server 能按 `archive_id` 增量消费一期归档，从原子发布的
...
`deploy/` 保存 Nginx、systemd 和 logrotate 生产模板。
...
15. `docs/15_第五期开发报告.md`
16. `docs/16_第六期暨终板验收报告.md`
17. `docs/17_部署与运维手册.md`
18. `docs/18_故障演练与排查.md`
19. `docs/19_核心调用链与面试演示.md`
20. `docs/20_简历能力与代码证据矩阵.md`
21. `docs/21_学习与面试准备路线.md`
22. `docs/22_面试回答参考.md`
23. `docs/23_项目完整技术说明.md`
...
完整发布门槛入口为 `scripts/acceptance.sh`，包含数据库幂等、Debug/Release、一期 Collector 到二期
HTTP 的跨项目 E2E、百万文档负载、格式/静态检查、覆盖率、Sanitizer 和 Valgrind。生产部署模板位于
`deploy/`，备份和校验分别使用 `scripts/backup.sh` 与 `scripts/verify_backup.sh`。详细步骤见
`docs/17_部署与运维手册.md`。

仓库根目录 `deploy-docker/` 提供与 DataStream 统一的容器化联调、只读一期归档挂载和跨项目一致
恢复流程。实际改进、试点 RTO/RPO 与尚需现场补齐的生产门槛见根目录
`SMT双项目Docker与单实例可靠性实际改进方案.md`。
